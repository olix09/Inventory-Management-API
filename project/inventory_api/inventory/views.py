...
class InventoryItemViewSet(viewsets.ModelViewSet):
    ...
    """
    queryset = InventoryItem.objects.all()  # <-- Added for router compatibility
    ...
    def low_stock(self, request):
        queryset = self.get_queryset().filter(quantity__lte=models.F('minimum_stock_level'))
    ...
    def overstocked(self, request):
        queryset = self.get_queryset().filter(quantity__gte=models.F('maximum_stock_level'))
        serializer = self.get_serializer(queryset, many=True)
        return Response({'count': queryset.count(), 'items': serializer.data})
    ...
    def summary(self, request):
        ...
        low_stock_count = queryset.filter(quantity__lte=models.F('minimum_stock_level')).count()
        out_of_stock_count = queryset.filter(quantity=0).count()
        categories_data = Category.objects.filter(items__owner=request.user).annotate(item_count=Count('items')).values('name', 'item_count')
        return Response({
    ...
    def adjust_quantity(self, request, pk=None):
        ...
        change_serializer = InventoryChangeSerializer(data=change_data, context={'request': request})
...
class InventoryChangeViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    """
    queryset = InventoryChange.objects.all()  # <-- Added for router compatibility
    ...
    def get_queryset(self):
        return InventoryChange.objects.filter(inventory_item__owner=self.request.user).select_related('inventory_item', 'changed_by')
